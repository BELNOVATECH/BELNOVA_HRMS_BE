...
def calculate_by_emp_id(
    ...
    try:
        ...
        leaves = db.query(
            ...
            LeaveRequest.status_id == 11,
            LeaveRequest.is_active == True
        ).all()

        leave_map[emp_id] = sum(float(l[0]) for l in leaves)

        return get_payroll_preview(emp, perc, leave_map, month, year)
...
def calculate_all(
    ...
    try:
        ...
        results = []
        for emp in employees:
            data = get_payroll_preview(emp, perc, leave_map, month, year)
            if data:
                results.append(data)
